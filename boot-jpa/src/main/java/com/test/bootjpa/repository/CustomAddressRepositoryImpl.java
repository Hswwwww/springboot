...
public class CustomAddressRepositoryImpl implements CustomAddressRepository {
    ...
    public List<Address> findAll() {
        ...
        /*
            ...
            - fetchFirst() : 단일 조회, 결과가 다중값이라도 처음것을 발생
            - fetchResult() : 페이징 정보 포함
            - fetchCount() : count쿼리로 변경 후 count 반환
    ...
    public List<AddressDTO> findNameAndAddress() {
        ...
        return jpaQueryFactory
                .select(Projections.constructor(AddressDTO.class, address1.name, address1.address)) //DTO 연결해주는 메소드?
                .from(address1)
                .fetch();
